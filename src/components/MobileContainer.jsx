...
export const MobileContainer = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-[#EAE4D9] text-vault-charcoal flex justify-center items-center sm:py-6 sm:px-4">
      {/* Mobile Frame Container (Max width ~412px for standard phone layout) */}
      <div className="w-full sm:max-w-[420px] min-h-screen sm:min-h-[860px] sm:h-[860px] bg-vault-paper sm:rounded-[40px] sm:border-[8px] sm:border-vault-borderDark shadow-2xl flex flex-col relative overflow-hidden">
        ...
        <header className="sticky top-0 z-30 bg-vault-paper/90 backdrop-blur-md px-5 pt-3 pb-2 flex items-center justify-between border-b border-vault-border select-none">
          <span className="text-xs font-display font-bold text-vault-charcoal tracking-wide">
            {time || '9:41'}
          </span>
          ...
          <div className="flex items-center gap-2.5 text-vault-muted">
            ...
            <Wifi className="w-3.5 h-3.5 text-vault-charcoal" />
            <Battery className="w-4 h-4 text-vault-charcoal" />
          </div>
        </header>

        {/* Top Header / User Profile Quick Toggle */}
        <div className="px-5 py-3 flex items-center justify-between bg-vault-paper border-b border-vault-border">
          <div className="flex items-center gap-3">
            <button 
              onClick={() => setActiveTab('profile')}
              className="relative rounded-full p-0.5 ring-2 ring-vault-terracotta/50 hover:ring-vault-terracotta transition-all"
            >
              <img 
                src={user.profilePic} 
                alt={user.name} 
            ...
            <div>
              <p className="text-[11px] text-vault-muted font-medium leading-none">Welcome back</p>
              <h2 className="text-sm font-bold text-vault-charcoal leading-tight mt-0.5">{user.name.split(' ')[0]}</h2>
            </div>
          </div>
        ...
        {toast && (
          <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-[380px] bg-vault-surface border border-vault-terracotta/40 text-vault-charcoal px-4 py-3 rounded-2xl shadow-xl flex items-center gap-3 animate-in fade-in slide-in-from-bottom-5">
            {toast.type === 'error' ? (
